...
export const FileUpload: React.FC<FileUploadProps> = ({ acceptedTypes, label, maxSizeMB = 5 }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <label
          className={`upload-label flex items-center justify-center w-full px-4 py-2 rounded-lg ${
            file
              ? 'bg-gray-300 cursor-not-allowed text-gray-500'
              : 'bg-[#D92027] text-white cursor-pointer hover:bg-[#f56a6e]'
          }`}
        >
        <input
          type="file"
      ...
      <style>{`.upload-label{width: 100%;} `}</style>
      {file && (
        <div className="flex items-center mt-4">
          <p className="text-lg text-gray-500 mr-4 mt-3">File: {file.name}</p>
          <button
            onClick={handleDelete}
            className="flex items-center bg-white text-red-500 py-1 px-3 rounded hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-400"
          >
      ...
      {error && <p className="text-red-500 mt-2 bg-red-100 p-3 rounded-md">{error}</p>}
      {success && <p className="text-green-500 mt-2 bg-green-100 p-3 rounded-md">File uploaded successfully!</p>}
    </div>
  );
};
