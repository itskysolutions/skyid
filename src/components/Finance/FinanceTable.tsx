...
import { DirectPayment } from "../../pages/Finance/DirectPayment";

interface FinanceTableDataItem {
  requestID: number;
  Date: string;
  userLevel: string;
  Name: string;
...
interface FinanceTableProps {
  Data: FinanceTableDataItem[];
}

export const FinanceTable = ({ Data }: FinanceTableProps) => {
          ...
          <table className="w-full table-auto border-collapse rounded-sm">
            <thead>
              <tr className="bg-gray-100">
                {["Request ID", "Date", "User Level", "Name", "Status", "Amount", "Actions"].map((header, idx) => (
                  <th
                    key={idx}
                    scope="col"
                      ...
                      <div className="flex gap-3">
                        <div> 
                            <CustomButton
                                ...
                                style="w-[185px] h-[44px]" 
                            />
                        </div>
                        <div> <DirectPayment/> </div>
                      </div>
                    </td>
                  </tr>
                ))
