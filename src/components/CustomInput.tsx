      ...
      render={({ field: { onBlur, onChange, value }, fieldState: { error } }) => (
        <div className={`flex flex-col ${mainContainer}`}>
          ...
          <div
            className={
              plainText
                ? `${plainStyle || "bg-[#E4F0FF] border border-[#E4F0FF] rounded-md font-extrabold flex-1"} p-[10px] font-inter text-base`
                : `flex flex-row items-center p-2 border border-gray-300 rounded-lg outline-none focus:bg-white ${style}`
            }
          >
            <input
              type={inputType}
